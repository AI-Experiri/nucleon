...
//!   q  = xn @ attn_q^T  -> 16 heads x head_dim
//!   k  = xn @ attn_k^T  -> 8 heads x head_dim (GQA)
//!   v  = xn @ attn_v^T  -> 8 heads x head_dim
...
//! Repeated per block. Final RMSNorm + tied lm head at the end.
//!
...
impl Qwen3 {
    ...
    pub fn forward(&self, ids: &[u32]) -> Result<Array, FamilyError> {
        ...
        for block in &self.blocks {
            // ----- attention sub-block -----
            let xn = fast::rms_norm(&x, &block.attn_norm, eps).map_err(FamilyError::forward)?;

            // Q, K, V projections via x @ W^T (W is stored [out, in]).
            let attn_q_t = block.attn_q.transpose().map_err(FamilyError::forward)?;
            let attn_k_t = block.attn_k.transpose().map_err(FamilyError::forward)?;
            let attn_v_t = block.attn_v.transpose().map_err(FamilyError::forward)?;
            let q = ops::matmul(&xn, &attn_q_t).map_err(FamilyError::forward)?;
            let k = ops::matmul(&xn, &attn_k_t).map_err(FamilyError::forward)?;
            let v = ops::matmul(&xn, &attn_v_t).map_err(FamilyError::forward)?;
            ...
            let q = fast::rms_norm(&q, &block.attn_q_norm, eps).map_err(FamilyError::forward)?;
            let k = fast::rms_norm(&k, &block.attn_k_norm, eps).map_err(FamilyError::forward)?;
            ...
            // MLX's SDPA handles GQA when n_heads > n_kv_heads and the
            // ratio is integer (here 16/8 = 2).
            ...
            let attn_output_t = block
                .attn_output
                .transpose()
                .map_err(FamilyError::forward)?;
            let delta = ops::matmul(&o, &attn_output_t).map_err(FamilyError::forward)?;

            x = ops::add(&x, &delta).map_err(FamilyError::forward)?;

            // ----- MLP sub-block (SwiGLU) -----
            let xn = fast::rms_norm(&x, &block.ffn_norm, eps).map_err(FamilyError::forward)?;
            let ffn_gate_t = block.ffn_gate.transpose().map_err(FamilyError::forward)?;
            let ffn_up_t = block.ffn_up.transpose().map_err(FamilyError::forward)?;
            let ffn_down_t = block.ffn_down.transpose().map_err(FamilyError::forward)?;

            let gate = ops::matmul(&xn, &ffn_gate_t).map_err(FamilyError::forward)?;
            let up = ops::matmul(&xn, &ffn_up_t).map_err(FamilyError::forward)?;
            let activated = silu(&gate).map_err(FamilyError::forward)?;
            let hidden_act = ops::multiply(&activated, &up).map_err(FamilyError::forward)?;
            let ffn_delta = ops::matmul(&hidden_act, &ffn_down_t).map_err(FamilyError::forward)?;

            x = ops::add(&x, &ffn_delta).map_err(FamilyError::forward)?;
        }
